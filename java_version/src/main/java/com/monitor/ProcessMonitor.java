...
public class ProcessMonitor implements Runnable {
    ...
    public void run() {
        ...
        try {
            ...
            Set<String> runningProcesses = ProcessHandle.allProcesses()
            ...
            if (monitoredApps != null) {
                for (String appName : monitoredApps) {
                    ...
                    if (isBrowser(lowerAppName)) {
                        boolean isBrowserRunning = checkProcessRunning(runningProcesses, lowerAppName);
                        String detectedWeb = isBrowserRunning ? detectWebActivity(activeWindowTitle) : null;

                        if (detectedWeb != null) {
                            handleAppActivity(detectedWeb, true, activeSessions, dbManager);
                            if (!activeSessions.containsKey(detectedWeb.toLowerCase())) {
                                hasNewSessionStarted = true;
                            }
                            isAnyActivityDetected = true;
                        }
                    } else {
                        boolean isRunning = checkProcessRunning(runningProcesses, lowerAppName);
                        handleAppActivity(appName, isRunning, activeSessions, dbManager);
                        if (isRunning && !activeSessions.containsKey(lowerAppName)) {
                            hasNewSessionStarted = true;
                        }
                        if(isRunning) isAnyActivityDetected = true;
                    }
                }
            }

            cleanupInactiveWebSessions(activeWindowTitle, runningProcesses);
    ...
    private boolean checkProcessRunning(Set<String> runningProcesses, String appName) {
        String lowerAppName = appName.toLowerCase();
        for (String process : runningProcesses) {
            if (process.endsWith(lowerAppName) || process.contains("\\" + lowerAppName) || process.contains("/" + lowerAppName)) {
                return true;
            }
        }
        return false;
